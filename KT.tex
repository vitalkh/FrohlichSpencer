...
Fix such i. Denote $d = min(d(\varrho_i), d(\varrho^*))$, so by (???????2.4) $ Md^\alpha \leq dist(\varrho_i, \varrho^*) $. Note that
$$
...
$$
Therefore, for $\alpha > 1$ and $M$ large enough $d=d(\varrho_i)$ holds. We then get from (iii) and (iv) that
...
Moreover, for $M$ large enough, we obtain from \eqref{label:eq448}
$$
