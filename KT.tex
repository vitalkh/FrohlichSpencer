...
$$
\midaLambda = exp\Big[-\frac{1}{2\beta}<\phi, (-\Delta_\Lambda)\phi>\Big]d\phi(k_0)\prod_{k \in \Lambda}d\phi(k),
$$
where:
\begin{enumerate}
\item $\beta > 0$.
\item $\Lambda_0 \subseteq \Lambda \subset \mathbb{Z}^2$ is some finite square.
...
\item $\Delta_\Lambda$ is the graph Laplacian with two possible boundary conditions:
\begin{enumerate}
...
\end{enumerate}
\begin{remark}
For notational simplicity, we denote $\mida = \midaLambda$, as $\Lambda, \Lambda_0$ are constant.
\end{remark}

\subsection{The Coulomb Gas}
...
Maybe a good title: State Representation as a Convex Combination 
...
\section{Chap 5??????????????}
\subsection{A Bound on $z(\beta, \varrho)$}
