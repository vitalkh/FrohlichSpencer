...
$$
\sum_{q=1}^N \xi_q = 1, \ \ \ and \ \ \ \abs{\xi_q}^{-1} < const e^{\eta^\prime q^2}
$$
...
\end{equation}
where $c_\ens^\prime > 0$, $\sum_\ens c_\ens^\prime = 1$, and each $\ens \in \mathscr{F}_{\vec{q}}$ has the following properties:
%TODO finish writing
\end{thm}
